...
resource "azurerm_lb" "lb" {
  for_each            = var.Lbs
  name                = "${var.prefix}-${each.value["suffix_name"]}-lb${each.value["id"]}"
  location            = local.location
  resource_group_name = var.resource_group_name
  sku                 = var.sku

  frontend_ip_configuration {
    name                          = "${var.prefix}-${each.value["suffix_name"]}-nic1-LBCFG"
...
resource "azurerm_lb_backend_address_pool" "lb_backend" {
  for_each = var.Lbs
  name     = "${var.prefix}-${each.value["suffix_name"]}-bckpool1"
...
resource "azurerm_lb_probe" "lb_probe" {
  ...
  name         = "${var.prefix}-${each.value["suffix_name"]}-probe${each.value["Id"]}"
